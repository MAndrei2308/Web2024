...
<body>
    ...
    <section class="souvenirs" id="souvenirs">
        ...
        <nav class="toc">
            ...
            <ul>
                <?php foreach (array_keys($productsByCountry) as $country): ?>
                    <li><a
                            href="#<?php echo strtolower(htmlspecialchars($country)); ?>"><?php echo htmlspecialchars($country); ?></a>
                    </li>
                <?php endforeach; ?>
            </ul>
        </nav>
        ...
        <?php foreach ($productsByCountry as $country => $products): ?>
            <h2 class="country" id="<?php echo strtolower(htmlspecialchars($country)); ?>">
                <?php echo htmlspecialchars($country); ?></h2>
                ...
                <?php foreach ($products as $product): ?>
                    <div class="souvenirs__box">
                        <form action="../endpoints/handle_favorite.php" method="POST" class="favorite-form">
                            ...
                            <button type="button" class="heart-checkbox"
                                onclick="toggleFavorite(<?php echo htmlspecialchars($product['id']); ?>, this)">
                                <div
                                    class="heart-icon <?php echo $favoriteController->isFavorite($userId, $product['id']) ? 'heart-icon-red' : ''; ?>">
                                </div>
                            </button>
                        ...
                        <div class="souvenirs__image"
                            onclick="location.href='Conectat_ProductDetail.php?id=<?php echo htmlspecialchars($product['id']); ?>'">
